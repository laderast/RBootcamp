...
```r
biopics %>%  #<1>
    filter(race_known == "Known") %>% #<2>
    mutate(poc_code = as.numeric(person_of_color)) #<3>
```
1. I took the `biopics` data, THEN
2. I `filter`ed it down with the `race_known == "Known"` criteria and THEN
3. I defined a new variable called `poc_code` with `mutate()`.
...
gender_box_office <- biopics %>% 
    filter(________) %>%
    group_by(_______) %>% 
    summarize(mean_bo_by_gender= mean(________))
...
#| echo: true
#| edit: false
biopics %>% select(movie_title=title, box_office)
```
...
threeVarTable <- biopics %>% 
  select(movieTitle=______, ______, _______)
...
```{r}
checkdown::check_question("`select()` works on columns, `filter()` works on rows",
                          options=c("`select()` works on booleans, whereas `filter()` works on all data types", "`select()` only works after `filter()`",
...
2. Then pipe that into a `ggplot()` statement that plots an x-y plot of `box_office` 
