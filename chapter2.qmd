...
ggplot(pets, aes(x= ______)) + geom_bar() + 
...
ggplot(pets, aes(x=animal, fill= _____)) + 
  geom_bar()
```
...
#| exercise: ex_4
ggplot(pets, aes(x=animal,fill=shotsCurrent)) + 
  geom_bar(position= ______, color="black")
```
...
#| exercise: ex_6
ggplot(pets, aes(x=name)) + geom_bar() + 
  facet_wrap(facets=~shotsCurrent, scale= _______) +
  theme(axis.text.x = element_text(angle=45))
```
...
checkdown::check_question("stratify our graph by another category" , 
                          options=c("stratify our graph by another category",
                                    ...
                                    ), random_answer_order = TRUE, type="radio")
```
...
```{webr}
#| exercise: ex_7
ggplot(pets, aes(x=ageCategory, fill= ______)) + 
    #what argument goes here?
    geom_bar(position = ________) +
    facet_wrap(facets = ______, scale = _____)
```
...
```{webr-r}
#| exercise: ex_8
ggplot(pets, aes(x= ______, y= _____)) + geom_boxplot()
```
...
```{webr}
#| exercise: ex_9
ggplot(pets, aes(x= _____, y= ______)) + geom_boxplot()
```
