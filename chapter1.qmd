...
Here's an example of a table. You can explore the table by row (by clicking on the numbers at the bottom), or look at the other columns (use the little triangle in the top right of the table).
...
We will be running commands on this `gap1992` table like below. Try running the code using the `Run Code` Button.

```{webr}
nrow(gap1992)
```

Notice that our function is called `nrow()`, and we call it on our `gap1992` table by putting it within the parentheses. 

### Exercise

- Run `colnames()` on the `gap1992` data to see what's in each column. 
- Then see how many rows there are in the dataset using `nrow()`. 
...
```{r}
checkdown::check_question("`linetype`", options = c("`x`", "`shape`", "`linetype`"), type="radio", random_answer_order = TRUE)
```

## Points versus lines

The great thing about `ggplot2` is that it's easy to swap geometric representations. 
